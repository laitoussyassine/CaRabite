...
export const getAllWorkshops = async (req, res) => {
  try {
    ...
    if (query) {
      ...
      }).populate('owner city'); // Populate 'owner' and 'city' fields with actual documents

    } else {
      // If no query is provided, fetch all workshops and populate 'owner' and 'city' fields
      workshops = await Workshop.find({}).populate('owner city');
    }
...
// Controller to fetch workshops with pagination
...
export const getWorkshopById = async (req, res) => {
  ...
  try {
    const workshop = await Workshop.findById(id).populate('city').exec();
